  ...
  return (
    <article className="grid gap-4 rounded-2xl border border-ca-border bg-white p-4 shadow-[var(--ca-shadow-soft)] lg:grid-cols-[230px_minmax(0,1fr)_190px] lg:gap-0 lg:p-0">
      <section className="lg:border-r lg:border-ca-border lg:p-5">
        ...
        <StatusBadge className="mt-4 lg:hidden" tracking={tracking} />
      </section>
      ...
      <section className="min-w-0 space-y-4 lg:p-5">
        ...
        <div className="grid gap-4 border-t border-ca-border pt-4 md:grid-cols-[minmax(0,1fr)_240px]">
          ...
          />
          <FulfillmentSummary order={order} tracking={tracking} />
        </div>
      </section>

      <aside className="flex flex-col justify-between gap-4 lg:p-5">
        <div className="flex items-start justify-between gap-3 lg:block">
          <StatusBadge className="hidden lg:inline-flex" tracking={tracking} />
        ...
        <div className="min-w-0">
          ...
          {item.partNumberSnapshot ? (
            <p className="mt-0.5 text-xs font-semibold text-ca-text-secondary">
              Parte: {item.partNumberSnapshot}
            </p>
          ) : null}
          <p className="mt-0.5 text-xs font-semibold text-ca-text-secondary">
            {item.brandSnapshot}
          </p>
          <VehicleFitment item={item} />
        </div>
      </div>
    );
  }
...
function VehicleFitment({ item }: { item: AccountOrderCardOrder["items"][number] }) {
  ...
  return (
    <p className="mt-1 text-xs font-semibold text-ca-text-secondary">
      Compatible: {fitment.make} {fitment.model} {fitment.yearFrom}-{fitment.yearTo}
    </p>
  );
}
  ...
  return (
    <div className="grid content-start gap-2 border-t border-ca-border pt-4 text-sm md:border-l md:border-t-0 md:pl-4 md:pt-0">
      <div className="flex items-start gap-2">
        ...
        <div>
          <p className="font-black text-ca-navy-950">{tracking.fulfillmentLabel}</p>
      ...
      <div className="flex items-start gap-2">
        ...
        <div>
          <p className="font-semibold text-ca-text-secondary">{tracking.dateLabel}:</p>
          <p className="font-black text-ca-navy-950">{formatTrackingDate(tracking.dateValue)}</p>
        </div>
      </div>
...
function OrderAction({
  ...
  action: NonNullable<ReturnType<typeof getOrderTrackingState>["secondaryAction"]> | ReturnType<typeof getOrderTrackingState>["primaryAction"];
  message: string;
  orderNumber: string;
  primary?: boolean;
}) {
  if (action === "Contactar asesor") {
    return (
      <WhatsAppCTA
        className="w-full"
