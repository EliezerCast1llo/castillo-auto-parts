...
export async function AccountOverviewHeader({
  createdAt,
  email,
  hasPassword,
  image,
  ...
  const displayEmail = email?.trim() || t("fallbackEmail");
          ...
          <div className="min-w-0">
            ...
            {createdAt ? (
              <p className="mt-3 text-xs font-semibold text-ca-text-secondary">
                {t("memberSince", { date: formatMonthYear(createdAt, locale) })}
              </p>
            ) : null}
          </div>
        </div>
...
function formatMonthYear(value: Date, locale: Locale) {
