...
| --- | --- |
| Fuerza bruta en login admin | Agregar rate limit por IP en server action de login. |
| Pago mock en produccion | Bloquear `PAYMENT_PROVIDER=mock` cuando `NODE_ENV=production`. |
| Orden publica adivinable | Agregar token guest firmado/hasheado para ver detalle de orden. |
| Sesion admin con secretos debiles | Rechazar secretos por defecto o faciles de adivinar; usar `SameSite=strict`; reducir expiracion. |
...
| Riesgo | Control recomendado |
| --- | --- |
| Headers de seguridad ausentes | Agregar `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, `X-Frame-Options` y HSTS en produccion. |
...
| Inputs sin limites claros | Agregar `.max()` en Zod para checkout y formularios admin. |
...
1. `codex/security-hardening-mvp`: bloquear pago mock en produccion, validar secretos admin, rate limit login y headers.
